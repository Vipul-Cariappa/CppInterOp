...
InterpreterInfo& getInterpInfo(compat::Interpreter* I) {
  std::lock_guard<std::mutex> Lock(InterpreterStackLock);
  if (!I)
    return *sInterpreters->back();
...
std::string GetFunctionSignature(TCppFunction_t func) {
  ...
  auto& IF = getInterpInfo(D);
  PrintingPolicy Policy = IF->getASTContext().getPrintingPolicy();
...
static TCppFuncAddr_t GetFunctionAddress(const FunctionDecl* FD,
  ...
  if (!llvm::isa<CXXConstructorDecl>(FD) && !llvm::isa<CXXDestructorDecl>(FD))
    return GetFunctionAddress(get_mangled_name(FD, ASTContext).c_str());
...
int get_wrapper_code(compat::Interpreter& I, const FunctionDecl* FD,
  ...
  ASTContext& Context = I.getASTContext();
  //
  ...
  if (!FD->isDefined(Definition)) {
    ...
    case FunctionDecl::TK_FunctionTemplateSpecialization: {
      ...
      if (!GetFunctionAddress(FD, I.getASTContext())) {
        if (!Pattern->hasBody()) {
          llvm::errs() << "TClingCallFunc::make_wrapper"
                       << ":"
                       << "Cannot make wrapper for a function template "
...
TInterp_t CreateInterpreter(const std::vector<const char*>& Args /*={}*/,
  ...
  sInterpreterASTMap->insert({&(*sInterpreters->back())->getASTContext(),
...
std::string LookupLibrary(const char* lib_name, TInterp_t I /*=nullptr*/) {
  ...
  return getInterpInfo(NULLPTR)->getDynamicLibraryManager()->lookupLibrary(
      lib_name);
}
...
static Decl* InstantiateTemplate(TemplateDecl* TemplateD,
                                 TemplateArgumentListInfo& TLI, Interpreter& I,
...
Decl* InstantiateTemplate(TemplateDecl* TemplateD,
                          ...
                          Interpreter& I, bool instantiate_body) {
  // Create a list of template arguments.
